...
func TestAccountFile(t *testing.T) {
	...
	record, _ := accounter.read(accounter.filename)

	if record.AccumulateBytes != 110 {
		t.Fatalf("Expected accumulate-bytes 110 but got %d", record.AccumulateBytes)
	}

	if record.CurrentBytes != 100 {
		t.Fatalf("Expected current-bytes 100 but got %d", record.CurrentBytes)
	}
}
